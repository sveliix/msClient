...
public class GuiCustomButton extends GuiButton
{
    ...
    {
        int i = this.getRange(msg, max);
        String s = this.getFirstStrings(max, msg);
        String s1 = "";

        for (int j = 0; j <= i; ++j)
        {
            LabyMod.getInstance().draw.drawCenteredString(s, x, y + j * 12);
            s1 = s1 + s;
            s = this.getFirstStrings(max, msg.replace(s1, ""));
        }
    }*/
