...
public class ModGuiTextField extends GuiTextField
{
    ...
    public ModGuiTextField(int componentId, FontRenderer fontrendererObj, int x, int y, int par5Width, int par6Height)
    {
    ...
    public void drawTextBox()
    {
        if (this.getVisible())
        {
            ...
            String s = this.getText().substring(this.lineScrollOffset);
            ...
            String s1 = this.fontRendererInstance.trimStringToWidth(s, this.getWidth());
            boolean flag = j >= 0 && j <= s1.length();
            boolean flag1 = this.isFocused() && this.cursorCounter / 6 % 2 == 0 && flag;
            int l = this.getEnableBackgroundDrawing() ? this.xPosition + 4 : this.xPosition;
            int i1 = this.getEnableBackgroundDrawing() ? this.yPosition + (this.height - 8) / 2 : this.yPosition;
            int j1 = l;
            ...
            if (s1.length() > 0)
            {
                String s2 = flag ? s1.substring(0, j) : s1;
            ...
            if (s1.length() > 0 && flag && j < s1.length())
            {
                j1 = this.fontRendererInstance.drawStringWithShadow(s1.substring(j), (float)j1, (float)i1, i);
            }
            ...
            if (k != j)
            {
                int l1 = l + this.fontRendererInstance.getStringWidth(s1.substring(0, k));
    ...
    {
        super.textboxKeyTyped(p_146201_1_, p_146201_2_);
