...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public static String login(final String email, final String password) {
        ...
        try {
        	...
        	login = false;
            return "§4§lAuthentication Server unavailable";
        } catch (AuthenticationException e) {
        	login = false;
            e.printStackTrace();
            if (e.getMessage().contains("Invalid username or password.")
                    || e.getMessage().toLowerCase().contains("account migrated")) {
                return "§4§lInvalid password!";
            }
            return "§4§lNo Connection to Mojang Server";
        } catch (NullPointerException e3) {
        	login = false;
            return "§4§lNULLPOINTER // CAN'T HAPPEN";
        }
    }

    public static void changeName(final String newName) {
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        Display.setTitle("MSClient loading...");
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    else
                    {
                        ...
                        for (ToggleSprintMod mod : ToggleSprintManager.getModules())
                        {
                        ...
                        if(this.gameSettings.MSGui.isPressed()) {
                        	Minecraft.getMinecraft().displayGuiScreen(new MSMainMenu(this.currentScreen));
                        }
            ...
            if (this.currentScreen == null && this.gameSettings.msgK.isPressed() && flag)
            {
                this.displayGuiScreen(new GuiChat("/msg Kruemelmomster "));
            }
            
            if (this.currentScreen == null && this.gameSettings.msgM.isPressed() && flag)
            {
                this.displayGuiScreen(new GuiChat("/msg MSES "));
            }
            
            /*for (KeyBinding k : Variables.msBindings) {
            	if (this.currentScreen == null && k.isPressed() && flag) {
            		this.displayGuiScreen(new GuiChat(k.getKeyDescription().substring(3)));
            	}
            }*/
