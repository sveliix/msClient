...
{
	public static EntityPlayerSP instance;
    ...
    public void onUpdate()
    {
    	...
    	MsEventHandler eventUpdate = new MsEventHandler();
    ...
    {
        if(main.onSendChatMessage(message)) {
    ...
    public void onLivingUpdate()
    {
        ...
        // FIXME Michel?
        this.customMovementInput.update(this.mc, (MovementInputFromOptions)this.movementInput, this.mc.thePlayer);
        ...
        // FIXME find raus was hier was ist
		...
		if(isSprintDisabled)
		{
            if(ToggleSneakMod.optionDoubleTap && this.onGround && !flag2 && this.movementInput.moveForward >= f && !this.isSprinting() && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness))
            {
               if(this.sprintToggleTimer <= 0 && !GameSettings.keyBindSprint.isKeyDown())
               {
            ...
            if(!this.isSprinting() && this.movementInput.moveForward >= f && flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && GameSettings.keyBindSprint.isKeyDown())
            {
               this.setSprinting(true);
               customMovementInput.UpdateSprint(true, false);
            }
		}else
		{
			...
			if(flag3 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) && !this.customMovementInput.sprintHeldAndReleased)
			{
		...
		if(ToggleSneakMod.optionEnableFlyBoost && this.capabilities.isFlying && GameSettings.keyBindSprint.isKeyDown() && this.capabilities.isCreativeMode)
		{
			this.capabilities.setFlySpeed(0.05F * (float)ToggleSneakMod.optionFlyBoostAmount);
			
			if(this.movementInput.sneak)	this.motionY -= 0.15D * (double)ToggleSneakMod.optionFlyBoostAmount;
			if(this.movementInput.jump)	this.motionY += 0.15D * (double)ToggleSneakMod.optionFlyBoostAmount;
		...
		if(this.isSprinting() && (this.movementInput.moveForward < f || this.isCollidedHorizontally || !flag3))
		{
			...
			if (customMovementInput.sprintHeldAndReleased == true || isSprintDisabled || customMovementInput.sprintDoubleTapped || this.capabilities.isFlying || this.isRiding())
			{
				customMovementInput.UpdateSprint(false, false);
			}
		}
		...
		if(this.isRidingHorse())
		{
			...
			{
				this.horseJumpPowerCounter = (this.horseJumpPowerCounter + 1);
				if(this.horseJumpPowerCounter == 0)
				{
			...
			{
				this.horseJumpPowerCounter = (this.horseJumpPowerCounter - 10);
				this.horseJumpPowerCounter = -10;
				((EntityPlayerSP)this).sendQueue.addToSendQueue(new C0BPacketEntityAction(this, C0BPacketEntityAction.Action.RIDING_JUMP, (int)(this.getHorseJumpPower() * 100.0F)));
			}
