...
public class GuiIngame extends Gui
{
    ...
    public GuiIngame(Minecraft mcIn)
    {
        ...
        MsEventHandler.onGuiIngameInit(this);
        this.func_175177_a();
    }
            ...
            for (PotionEffect potioneffect : this.mc.thePlayer.getActivePotionEffects())
            {
                ...
                if (potion.hasStatusIcon())
                {
                    ...
                    GL11.glPushMatrix(); //Start new matrix
      		        GL11.glScalef(0.5F, 0.5F, 0.5F); //scale it to 0.5 size on each side. Must be float e.g.: 2.0F
                    this.drawTexturedModalRect(Math.round((i + 2)/0.5F), Math.round((j + 9)/0.5F), Math.round((0 + i1 % 8 * 18)), Math.round((198 + i1 / 8 * 18)), Math.round(18), Math.round(18));
                ...
                GL11.glPushMatrix(); //Start new matrix
  		        GL11.glScalef(0.5F, 0.5F, 0.5F); //scale it to 0.5 size on each side. Must be float e.g.: 2.0F
                mc.fontRendererObj.drawStringWithShadow(s1, Math.round(((float)(i + 8 + 5))/0.5F), Math.round(((float)(j + 8))/0.5F), 16777215);
                String s = Potion.getDurationString(potioneffect);
                mc.fontRendererObj.drawStringWithShadow(s, Math.round(((float)(i + 8 + 5))/0.5F), Math.round(((float)(j + 5 + 10))/0.5F), 16777215);
                GL11.glPopMatrix(); //End this matrix
                j += l;
    ...
    {    	
    	...
    	this.mc.thePlayer.setCustomNameTag("MSES");
        ...
        if(this.mc.gameSettings.showDebugInfo) {
        	...
        	MsEventHandler.onRenderGameOverlay(this);
        }
                
        RenderTextToHUD.instance.RenderGameOverlayEvent();
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        GlStateManager.disableLighting();
        GlStateManager.enableAlpha();
    }
